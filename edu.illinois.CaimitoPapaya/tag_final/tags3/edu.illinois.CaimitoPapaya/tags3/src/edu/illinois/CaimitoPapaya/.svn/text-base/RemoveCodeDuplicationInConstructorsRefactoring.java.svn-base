...
public class RemoveCodeDuplicationInConstructorsRefactoring extends Refactoring {
	...
	public List union(List parameters1, List parameters2) {
	 ...
	 */
	@SuppressWarnings( {"unchecked", "rawtypes" } )
	public MethodDeclaration createNewConstructor(TypeDeclaration td) {
	 ...
	 */
	@SuppressWarnings({ "rawtypes" })
	public void copyParamsAndMoveExpressions(
	...
	private ConstructorInvocation getconstructorInvocationDefaults(
		...
		if(currType.isPrimitiveType()) {
			...
			if(primType.getPrimitiveTypeCode() == 
				...
				conInvoc.arguments().add(ast.newBooleanLiteral(false));
			...
			// Check if it's one of the number literals.
					...
					primType.getPrimitiveTypeCode() ==
					PrimitiveType.SHORT)
				conInvoc.arguments().add(ast.newNumberLiteral(Integer.toString(0)));
			...
			// Check if it's one of the decimal number literals.
					...
					primType.getPrimitiveTypeCode() ==
					PrimitiveType.FLOAT)
				conInvoc.arguments().add(ast.newNumberLiteral(Double.toString(0.0)));
		}
		
		// Otherwise
		else
			conInvoc.arguments().add(ast.newNullLiteral());
	...
	private void refactorSubsetParams(
		...
		constructor1.getBody().statements().add(newConInvoc);
	}
